...
export function CompletionCounter() {
  ...
  return (
    <div className="completion-counter" ref={containerRef}>
      ...
      <button type="button" aria-label="How completion counting works" aria-expanded={isOpen} aria-controls="completion-count-explanation" aria-describedby={isOpen ? "completion-count-explanation" : undefined} onClick={() => setIsOpen((open) => !open)}>
        ?
      </button>
      {isOpen && (
        <div className="completion-count-popover" id="completion-count-explanation" role="tooltip">
